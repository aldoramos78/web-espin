...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const isEn = lang === 'en';
  return {
    title: isEn ? 'Complete Ecosystem | espín' : 'Ecosistema Completo | espín',
    description: isEn ? 'Premium identity, high-performance web infrastructure, and AI automation. A complete intervention for absolute control of your company.' : 'Identidad premium, infraestructura web de alto rendimiento y automatización con IA. Una intervención completa para el control absoluto de tu empresa.',
  };
}

export default async function EcosistemaPage({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const isEn = lang === 'en';
  const trilogiaData = [
    { 
      letter: "/01", 
      title: isEn ? "IDENTITY" : "IDENTIDAD", 
      desc: isEn ? "We design from scratch or rebuild your brand so your image justifies your prices and transmits maximum authority." : "Diseñamos desde cero o reconstruimos tu marca para que tu imagen justifique tus precios y transmita máxima autoridad.", 
    ...
    { 
      letter: "/02", 
      title: isEn ? "INFRASTRUCTURE" : "INFRAESTRUCTURA", 
              ...
              <div className="overflow-hidden">
                ...
                <div className="overflow-hidden pb-4">
                  <ScrollReveal variant="textReveal" delay={0.2} className="">
                    {isEn ? "COMPLETE" : "COMPLETO"}<span className="">.</span>
                  </ScrollReveal>
                </div>
            </h1>
 ...
 <h2 className="font-clash font-light text-[4vw] sm:text-lg md:text-2xl lg:text-3xl text-[#F5B700] uppercase whitespace-nowrap"> 
                      01 / {isEn ? "THE TRILOGY" : "LA TRILOGÍA"}
                    </h2>
                  </div>
                </ScrollReveal>
              </div>
 ...
 <h2 className="font-clash font-semibold text-[7vw] sm:text-3xl md:text-5xl lg:text-[4rem] text-white mb-8 md:mb-12 uppercase break-words"> 
                  {isEn ? <>GOVERNANCE AND <span className="text-[#F5B700]">MANDATORY</span> MAINTENANCE.</> : <>GOBERNANZA Y MANTENIMIENTO <span className="text-[#F5B700]">OBLIGATORIO</span>.</>}
                </h2>
              </ScrollReveal>
              <ScrollReveal delay={0.2}>
                <p className="font-inter text-zinc-400 text-lg md:text-xl lg:text-2xl leading-relaxed max-w-3xl">
                  {isEn ? "We do not deliver technology only to disappear later. This ecosystem includes continuous all-risk insurance. We take monthly care of the security, updates, and evolution of your Artificial Intelligence so that you only focus on leading." : "No entregamos tecnología para luego desaparecer. Este ecosistema incluye un seguro a todo riesgo continuo. Nos encargamos mensualmente de la seguridad, las actualizaciones y la evolución de tu Inteligencia Artificial para que tú solo te centres en dirigir."}
                </p>
              </ScrollReveal>
            </div>
          </ScrollReveal>
              ...
              <div className="mb-10 md:mb-16 w-full">
                <ScrollReveal variant="slideRight">
                  <div className="flex items-center gap-3 md:gap-4">
                    <div className="w-8 md:w-16 h-[2px] bg-[#F5B700] shrink-0"></div>
 <h2 className="font-clash font-light text-[4vw] sm:text-lg md:text-2xl lg:text-3xl text-[#F5B700] uppercase whitespace-nowrap"> 
                      03 / {isEn ? "THE DECISION" : "LA DECISIÓN"}
                    </h2>
                  </div>
                </ScrollReveal>
              </div>
            ...
            <ScrollReveal>
 <h2 className="font-clash font-semibold text-[7vw] sm:text-3xl md:text-5xl lg:text-[4rem] text-white mb-8 md:mb-12 uppercase break-words"> 
                {isEn ? <>DELEGATE TECHNOLOGY.<br /> <span className="text-[#F5B700]">LEAD</span> YOUR MARKET.</> : <>DELEGA LA TECNOLOGÍA.<br /> <span className="text-[#F5B700]">LIDERA</span> TU MERCADO.</>}
              </h2>
            </ScrollReveal>
            <ScrollReveal delay={0.2}>
              <p className="font-inter font-light text-zinc-400 text-lg md:text-xl lg:text-2xl leading-relaxed mb-16 max-w-3xl">
                {isEn ? "We operate with a very limited number of clients to guarantee an uncompromising standard." : "Operamos con un número muy limitado de clientes para garantizar un nivel de exigencia absoluto."}
              </p>
            </ScrollReveal>
            <ScrollReveal delay={0.4}>
              <ContactTrigger servicio="ecosistema" className="inline-block cursor-pointer">
                <button type="button" className="rings-btn">
                  <i></i><i></i><i></i>
                  <span>{isEn ? "Talk to a Consultant" : "Hablar con un Consultor"}</span>
                  <svg className="arr" viewBox="0 0 24 24" aria-hidden="true"><path d="M7 17L17 7M9 7h8v8"/></svg>
                </button>
              </ContactTrigger>
              </ScrollReveal>
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                {isEn ? "TOTAL INTERVENTION" : "INTERVENCIÓN TOTAL"}
              </span>
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                {isEn ? "ZERO PATCHES" : "CERO PARCHES"}
              </span>
              <svg aria-hidden="true" viewBox="0 0 24 24" className="text-[#F5B700] w-8 h-8 md:w-12 md:h-12 shrink-0 pb-1" style={{ stroke: 'currentColor', strokeWidth: 1.5, fill: 'none' }}>
                <line x1="12" y1="2" x2="12" y2="22" />
                <line x1="2" y1="12" x2="22" y2="12" />
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                {isEn ? "ABSOLUTE CONTROL" : "CONTROL ABSOLUTO"}
              </span>
              <svg aria-hidden="true" viewBox="0 0 24 24" className="text-[#F5B700] w-8 h-8 md:w-12 md:h-12 shrink-0 pb-1" style={{ stroke: 'currentColor', strokeWidth: 1.5, fill: 'none' }}>
                <line x1="12" y1="2" x2="12" y2="22" />
                <line x1="2" y1="12" x2="22" y2="12" />
