...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const isEn = lang === 'en';
  return {
    title: isEn ? 'AI Agents & Automation | espín' : 'Agentes y Automatización | espín',
    description: isEn ? 'We integrate AI assistants and automate mechanical tasks to reduce costs and multiply performance.' : 'Integramos asistentes de IA y automatizamos tareas mecánicas para reducir costes y multiplicar el rendimiento.',
  };
}

export default async function AgentesLanding({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const isEn = lang === 'en';
  const problemaData = [
    { letter: "/A", title: isEn ? "MANUAL BOTTLENECKS" : "CUELLOS DE BOTELLA MANUALES", desc: isEn ? "Your team loses dozens of hours a week on repetitive, mechanical, and management tasks that hold back the real growth of the company." : "Tu equipo pierde decenas de horas a la semana en tareas repetitivas, mecánicas y de gestión que frenan el crecimiento real de la empresa.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
    { letter: "/B", title: isEn ? "CLIENT LEAKAGE" : "FUGA DE CLIENTES", desc: isEn ? "If a potential client asks for information at 10 PM and no one answers instantly, the next day they will have closed with your competition." : "Si un cliente potencial pide información a las diez de la noche y nadie le responde al instante, al día siguiente habrá cerrado con tu competencia.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6" /></svg> },
    { letter: "/C", title: isEn ? "INFLATED COSTS" : "COSTES INFLADOS", desc: isEn ? "You are paying high salaries to highly trained people to do copy-and-paste work that a machine can do in seconds." : "Estás pagando sueldos elevados a personas altamente capacitadas para que hagan un trabajo de copia y pega que una máquina puede hacer en segundos.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> }
  ];

  const pilaresData = [
    { letter: "P.01", title: isEn ? "ABSOLUTE AVAILABILITY" : "DISPONIBILIDAD ABSOLUTA", desc: isEn ? "Your company never sleeps, never gets sick, and never takes vacations. Total operability 365 days a year." : "Tu empresa nunca duerme, nunca se pone enferma y nunca coge vacaciones. Operatividad total los 365 días del año.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg> },
    { letter: "P.02", title: isEn ? "PINPOINT PRECISION" : "PRECISIÓN MILIMÉTRICA", desc: isEn ? "We eradicate human error from administrative tasks, billing, and data management." : "Erradicamos el error humano de las tareas administrativas, la facturación y la gestión de datos.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z" /></svg> },
    { letter: "P.03", title: isEn ? "IMMEDIATE SCALABILITY" : "ESCALABILIDAD INMEDIATA", desc: isEn ? "If today you receive 10 emails and tomorrow you receive 10,000, the system handles them all at once with the same quality. Without hiring anyone else." : "Si hoy recibes 10 correos y mañana recibes 10.000, el sistema los atiende todos a la vez con la misma calidad. Sin contratar a nadie más.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg> }
  ];
          ...
          <div className="absolute inset-0 z-0 bg-gradient-to-b from-transparent via-black/40 to-black"></div>
            ...
            <ScrollReveal delay={0.15}>
              <div className="font-space-mono text-[#F5B700] text-[10px] md:text-xs tracking-[0.2em] mb-6 uppercase">
                {isEn ? "[MODULE 02] // STATUS: ACTIVE // TECHNICAL REPORT: AGENTS" : "[MÓDULO 02] // ESTADO: ACTIVO // REPORTE TÉCNICO: AGENTES"}
              </div>
            </ScrollReveal>
...
{isEn ? <>We make your <span className="text-[#F5B700]">company</span> work on its own. We integrate virtual assistants and automate internal <span className="text-[#F5B700]">processes</span> so your business operates, serves, and sells <span className="text-[#F5B700]">24 hours</span> a day with zero margin for human error.</> : <>Hacemos que tu <span className="text-[#F5B700]">empresa</span> trabaje sola. Integramos asistentes virtuales y automatizamos <span className="text-[#F5B700]">procesos</span> internos para que tu negocio opere, atienda y venda las <span className="text-[#F5B700]">24 horas</span> del día sin margen de error humano.</>}
</p>
                </ScrollReveal>
              </div>
              ...
              <ScrollReveal delay={0.8}>
                <ContactTrigger servicio="agentes" className="inline-block cursor-pointer">
                  <button type="button" className="rings-btn">
                    <i></i><i></i><i></i>
                    <span>{isEn ? "Request AI Audit" : "Solicitar Auditoría IA"}</span>
 ...
 <h2 className="font-clash font-light text-[4vw] sm:text-lg md:text-2xl lg:text-3xl text-[#F5B700] uppercase whitespace-nowrap"> 
              ...
              <div className="leading-[1.1] md:leading-[1.1] flex flex-col">
                {[
                  { id: "01", title: isEn ? "Inefficiency Mapping" : "Mapeo de Ineficiencias", desc: isEn ? "We audit your day-to-day to detect which manual processes are costing you time and money." : "Auditamos tu día a día para detectar qué procesos manuales te están costando tiempo y dinero." },
                  { id: "02", title: isEn ? "System Training" : "Entrenamiento del Sistema", desc: isEn ? "We configure Artificial Intelligence to speak, act, and make decisions with exactly the same rules and voice as your business." : "Configuramos la Inteligencia Artificial para que hable, actúe y tome decisiones exactamente con las mismas reglas y voz de tu negocio." },
                                    { id: "03", title: isEn ? "Silent Deployment" : "Despliegue Silencioso", desc: isEn ? "We integrate automations into your current tools (WhatsApp, email, CRM) without interrupting daily operations." : "Integramos los automatismos en tus herramientas actuales (WhatsApp, correo, CRM) sin interrumpir la operativa diaria." },
                ].map((phase, i) => (
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "AUTONOMOUS SYSTEMS" : "SISTEMAS AUTÓNOMOS"}
                </span>
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "SMART SUPPORT" : "ATENCIÓN INTELIGENTE"}
                </span>
                <svg aria-hidden="true" viewBox="0 0 24 24" className="text-[#F5B700] w-8 h-8 md:w-12 md:h-12 shrink-0 pb-1" style={{ stroke: 'currentColor', strokeWidth: 1.5, fill: 'none' }}>
                  <line x1="12" y1="2" x2="12" y2="22" />
                  <line x1="2" y1="12" x2="22" y2="12" />
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "COST REDUCTION" : "REDUCCIÓN DE COSTES"}
                </span>
                <svg aria-hidden="true" viewBox="0 0 24 24" className="text-[#F5B700] w-8 h-8 md:w-12 md:h-12 shrink-0 pb-1" style={{ stroke: 'currentColor', strokeWidth: 1.5, fill: 'none' }}>
                  <line x1="12" y1="2" x2="12" y2="22" />
                  <line x1="2" y1="12" x2="22" y2="12" />
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "MAXIMUM EFFICIENCY" : "MÁXIMA EFICIENCIA"}
                </span>
