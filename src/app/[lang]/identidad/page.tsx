...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const isEn = lang === 'en';
  return {
    title: isEn ? 'Identity & Brand Creation | espín' : 'Identidad y Creación de Marca | espín',
    description: isEn ? 'We design brands from scratch and renew stagnant companies so they project exclusivity, authority, and total confidence.' : 'Diseñamos marcas desde cero y renovamos empresas estancadas para que proyecten exclusividad, autoridad y confianza total.',
  };
}

export default async function EspinLanding({ params }: { params: Promise<{ lang: string }> }) {
  const { lang } = await params;
  const isEn = lang === 'en';
  const problemaData = [
    { letter: "/A", title: isEn ? "DEVALUED PERCEPTION" : "PERCEPCIÓN DEVALUADA", desc: isEn ? "If your website, documents, or logo look cheap, the client will assume your service is too, and will demand discounts before you even start talking." : "Si tu web, tus documentos o tu logotipo parecen baratos, el cliente asumirá que tu servicio también lo es, y te exigirá rebajas antes de empezar a hablar.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path strokeLinecap="round" strokeLinejoin="round" d="M13 8l-3 4h4l-2 5"/></svg> },
    { letter: "/B", title: isEn ? "NOISE AND CONFUSION" : "RUIDO Y CONFUSIÓN", desc: isEn ? "You are just another one in a sea of competitors who say and do exactly the same thing. You are not giving your client any visual reason to choose you." : "Eres uno más en un mar de competidores que dicen y hacen exactamente lo mismo. No le estás dando a tu cliente ningún motivo visual para elegirte a ti.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M14 6a6 6 0 1 0 0 12M5 10h8M5 14h8" /></svg> },
    { letter: "/C", title: isEn ? "CORPORATE INCONSISTENCY" : "INCOHERENCIA CORPORATIVA", desc: isEn ? "Every social network, invoice, or presentation has a different style, color, and typography. That conveys chaos, disorganization, and a serious lack of professionalism." : "Cada red social, factura o presentación tiene un estilo, color y tipografía distintos. Eso transmite caos, desorganización y una grave falta de profesionalidad.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M3 7l6 6 4-4 8 8M21 17v-6M21 17h-6" /></svg> },
  ];

  const pilaresData = [
    { letter: "P.01", title: isEn ? "STRATEGIC AESTHETICS" : "ESTÉTICA ESTRATÉGICA", desc: isEn ? "We're not in the business of art. We're in the business of results. We design with a single commercial objective: to drastically increase the perceived value of your company." : "No hacemos arte, hacemos negocios. Diseñamos con un único objetivo comercial: aumentar drásticamente el valor percibido de tu empresa.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M3 17l6-6 4 4 8-8M21 7v6M21 7h-6" /></svg> },
    { letter: "P.02", title: isEn ? "PARTNERS, NOT TECHNICIANS" : "SOCIOS, NO TÉCNICOS", desc: isEn ? "Our client is not looking for an IT guy to put out fires. They understand that software is the main engine of their profitability and demand a technological partner capable of sustaining and shielding their long-term growth." : "Nuestro cliente no busca un informático para apagar fuegos. Entiende que el software es el motor principal de su rentabilidad y exige un partner tecnológico capaz de sostener y blindar su crecimiento a largo plazo.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg> },
    { letter: "P.03", title: isEn ? "BUSINESS ENGINEERING" : "INGENIERÍA DE NEGOCIO", desc: isEn ? "Every line of code, every server optimization, and every architecture we deploy has a single non-negotiable objective: to eradicate inefficiencies and increase your profit." : "Cada línea de código, cada optimización de servidor y cada arquitectura que desplegamos tiene un único objetivo innegociable: erradicar ineficiencias y aumentar tu beneficio.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" /></svg> },
  ];
          ...
          <div className="absolute inset-0 z-0 bg-gradient-to-b from-transparent via-black/40 to-black"></div>
            ...
            <div className="mb-8 md:mb-12">
              <ScrollReveal delay={0.1}>
                <Link href={isEn ? `/${lang}` : "/"} className="inline-block">
            ...
            <ScrollReveal delay={0.15}>
              <div className="font-space-mono text-[#F5B700] text-[10px] md:text-xs tracking-[0.2em] mb-6 uppercase">
                {isEn ? "[MODULE 03] // STATUS: ACTIVE // TECH REPORT: BRANDING" : "[MÓDULO 03] // ESTADO: ACTIVO // REPORTE TÉCNICO: IDENTIDAD"}
              </div>
            </ScrollReveal>

 <h1 className="hero-stroke font-clash font-bold uppercase mb-8 md:mb-12 text-[10vw] sm:text-[8vw] md:text-6xl lg:text-[5rem] xl:text-[6.5rem] break-words leading-[1.1] md:leading-[1.1] flex flex-col w-full max-w-5xl border-l-0 md:border-l-[6px] border-[#F5B700] pl-0 md:pl-10"> 
                <div className="overflow-hidden">
                    <ScrollReveal variant="textReveal" delay={0.1} className="">
                      {isEn ? "IDENTITY &" : "IDENTIDAD Y"}
                    </ScrollReveal>
                  </div>
                  <div className="overflow-hidden pb-4">
                    <ScrollReveal variant="textReveal" delay={0.2} className="">
                      {isEn ? "BRANDING" : "CREACIÓN DE MARCA"}<span className="">.</span>
                    </ScrollReveal>
                  </div>
              </h1>
              ...
              <div className="overflow-hidden mb-12 md:mb-20 pt-4">
                <ScrollReveal delay={0.3}>
                  ...
                  <p className="font-inter font-normal text-zinc-400 text-base md:text-xl lg:text-2xl max-w-4xl leading-relaxed md:leading-tight tracking-tight">
                    {isEn ? <>The <span className="text-[#F5B700]">image</span> of your business must justify your prices. We design brands from scratch and renew stagnant companies so they project <span className="text-[#F5B700]">exclusivity</span>, authority, and total <span className="text-[#F5B700]">confidence</span> in your sector.</> : <>La <span className="text-[#F5B700]">imagen</span> de tu negocio debe justificar tus precios. Diseñamos marcas desde cero y renovamos empresas estancadas para que proyecten <span className="text-[#F5B700]">exclusividad</span>, autoridad y <span className="text-[#F5B700]">confianza</span> total en tu sector.</>}
                  </p>
                </ScrollReveal>
              </div>
              ...
              <ScrollReveal delay={0.8}>
                <ContactTrigger servicio="marca" className="inline-block cursor-pointer">
                  <button type="button" className="rings-btn">
                    <i></i><i></i><i></i>
                    <span>{isEn ? "Request Visual Audit" : "Solicitar Auditoría Visual"}</span>
            ...
            <div className="absolute bottom-8 right-6 md:bottom-12 md:right-12 z-20">
              <ScrollReveal delay={1.0}>
                <Link href={isEn ? "/en/manifesto" : "/es/manifiesto"}
 className="relative inline-block text-[#888888] font-clash font-semibold text-[10px] md:text-xs pb-1 group transition-colors hover:text-white whitespace-nowrap" 
                >
                  {isEn ? "DOC.00 / MANIFESTO" : "DOC.00 / MANIFIESTO"}
 ...
 <h2 className="font-clash font-light text-[4vw] sm:text-lg md:text-2xl lg:text-3xl text-[#F5B700] uppercase whitespace-nowrap"> 
                        01 / {isEn ? "THE PROBLEM" : "EL PROBLEMA"}
                      </h2>
                    </div>
                  </ScrollReveal>
                </div>
 ...
 <h2 className="font-clash font-light text-[4vw] sm:text-lg md:text-2xl lg:text-3xl text-[#F5B700] uppercase whitespace-nowrap"> 
              ...
              <div className="leading-[1.1] md:leading-[1.1] flex flex-col">
                {[
                  { id: "01", title: isEn ? "Positioning Analysis" : "Análisis de Posicionamiento", desc: isEn ? "We study how the market perceives you today and define the exact space you must occupy to charge what your service is really worth." : "Estudiamos cómo te percibe el mercado hoy y definimos el espacio exacto que debes ocupar para cobrar lo que realmente vale tu servicio." },
                  { id: "02", title: isEn ? "Visual System" : "Sistema Visual", desc: isEn ? "We create logos, colors, and typography that are not only aesthetic, but psychologically designed to attract high-purchasing-power clients." : "Creamos logotipos, colores y tipografías que no solo son estéticos, sino que están diseñados psicológicamente para atraer a clientes de alto poder adquisitivo." },
                                    { id: "03", title: isEn ? "Corporate Deployment" : "Despliegue Corporativo", desc: isEn ? "We apply your new visual armor to all the company's touchpoints, from the web to internal documents." : "Aplicamos tu nueva armadura visual a todos los puntos de contacto de la empresa, desde la web hasta los documentos internos." },
                ].map((phase, i) => (
              ...
              <div className="mb-10 md:mb-16 w-full">
                  <ScrollReveal variant="slideRight">
                    <div className="flex items-center gap-3 md:gap-4">
                      <div className="w-8 md:w-16 h-[2px] bg-[#F5B700] shrink-0"></div>
 <h2 className="font-clash font-light text-[4vw] sm:text-lg md:text-2xl lg:text-3xl text-[#F5B700] uppercase whitespace-nowrap"> 
                        03 / {isEn ? "THE DOCTRINE" : "LA DOCTRINA"}
                      </h2>
                    </div>
                  </ScrollReveal>
                </div>
 ...
 <div className="text-6xl md:text-7xl text-white mb-2 font-clash font-semibold flex items-baseline"> 
                  ...
                  <div className="text-zinc-500 text-[10px] md:text-xs font-mono uppercase tracking-widest">
                    {isEn ? "SHIELDED AUTHORITY" : "AUTORIDAD BLINDADA"}
                  </div>
                </div>
                ...
                <div className="mt-auto">
                  ...
                  <span className="text-[#F5B700] text-[10px] md:text-xs font-mono tracking-widest uppercase mb-6 block">{isEn ? "Verified Impact" : "Impacto Verificado"}</span>
                  <ul className="space-y-4 font-inter font-normal">
                    <li className="flex items-start text-zinc-400 text-sm">
                      ...
                      <span>{isEn ? "Total coherence across all your commercial channels." : "Coherencia total en todos tus canales comerciales."}</span>
                    </li>
                  </ul>
                </div>
              </ScrollReveal>
 ...
 <div className="text-4xl sm:text-5xl md:text-7xl text-white mb-2 font-clash font-semibold flex items-baseline"> 
                  ...
                  <div className="text-zinc-500 text-[10px] md:text-xs font-mono uppercase tracking-widest">
                    {isEn ? "AVERAGE TICKET INCREASE" : "AUMENTO DEL TICKET MEDIO"}
                  </div>
                </div>
                ...
                <div className="mt-auto">
                  ...
                  <ul className="space-y-4 font-inter font-normal">
                    <li className="flex items-start text-zinc-400 text-sm">
                      <svg aria-hidden="true" className="w-4 h-4 text-[#F5B700] mr-4 mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7"></path></svg>
                      <span>{isEn ? "Your clients stop arguing over the price when your company's perception is indisputably premium." : "Tus clientes dejan de discutir el precio cuando la percepción de tu empresa es indiscutiblemente premium."}</span>
                    </li>
                  </ul>
                </div>
              </ScrollReveal>
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "VISUAL AUTHORITY" : "AUTORIDAD VISUAL"}
                </span>
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "IMMEDIATE TRUST" : "CONFIANZA INMEDIATA"}
                </span>
                <svg aria-hidden="true" viewBox="0 0 24 24" className="text-[#F5B700] w-8 h-8 md:w-12 md:h-12 shrink-0 pb-1" style={{ stroke: 'currentColor', strokeWidth: 1.5, fill: 'none' }}>
                  <line x1="12" y1="2" x2="12" y2="22" />
                  <line x1="2" y1="12" x2="22" y2="12" />
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "JUSTIFIED PRICES" : "PRECIOS JUSTIFICADOS"}
                </span>
                <svg aria-hidden="true" viewBox="0 0 24 24" className="text-[#F5B700] w-8 h-8 md:w-12 md:h-12 shrink-0 pb-1" style={{ stroke: 'currentColor', strokeWidth: 1.5, fill: 'none' }}>
                  <line x1="12" y1="2" x2="12" y2="22" />
                  <line x1="2" y1="12" x2="22" y2="12" />
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "PREMIUM POSITIONING" : "POSICIONAMIENTO PREMIUM"}
                </span>
                <svg aria-hidden="true" viewBox="0 0 24 24" className="text-[#F5B700] w-8 h-8 md:w-12 md:h-12 shrink-0 pb-1" style={{ stroke: 'currentColor', strokeWidth: 1.5, fill: 'none' }}>
                  <line x1="12" y1="2" x2="12" y2="22" />
                  <line x1="2" y1="12" x2="22" y2="12" />
