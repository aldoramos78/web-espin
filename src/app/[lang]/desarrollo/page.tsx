...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const isEn = lang === 'en';
  return {
    title: isEn ? 'High-Performance Web Development | espín' : 'Desarrollo Web de Alto Rendimiento | espín',
    description: isEn ? 'We build unbreakable, ultra-fast web infrastructures optimized to scale.' : 'Construimos infraestructuras web inquebrantables, ultrarrápidas y optimizadas para escalar.',
  };
}
...
export default async function EspinLanding({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const isEn = lang === 'en';
  const problemaData = [
    { letter: "/A", title: isEn ? "EXPOSED SYSTEM" : "SISTEMA EXPUESTO", desc: isEn ? "Security breaches, obsolete web templates, vulnerable third-party plugins, broken links. We eradicate vulnerable technology and deploy unbreakable digital fortresses." : "Brechas de seguridad, plantillas web obsoletas, plugins de terceros vulnerados, enlaces rotos. Extirpamos tecnología vulnerable y desplegamos fortalezas digitales inquebrantables.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path strokeLinecap="round" strokeLinejoin="round" d="M13 8l-3 4h4l-2 5"/></svg> },
    { letter: "/B", title: isEn ? "CAPITAL LEAKS" : "FUGAS DE CAPITAL", desc: isEn ? "A slow, down, or obsolete system makes your client go to the competition. We optimize the speed of your ecosystem so that retention is maximum." : "Un sistema lento, caído u obsoleto hace que tu cliente se vaya a la competencia. Optimizamos la velocidad de tu ecosistema para que la retención sea máxima.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M14 6a6 6 0 1 0 0 12M5 10h8M5 14h8" /></svg> },
    { letter: "/C", title: isEn ? "BRAND DEVALUATION" : "DEVALUACIÓN DE MARCA", desc: isEn ? "An outdated interface projects decadence. Our digital image is the gateway to our home. We rebuild your digital presence to shield your authority." : "Una interfaz anticuada proyecta decadencia. Nuestra imagen digital es la puerta de entrada a nuestra casa. Reconstruimos tu presencia digital para blindar tu autoridad.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M3 7l6 6 4-4 8 8M21 17v-6M21 17h-6" /></svg> },
  ];

  const pilaresData = [
    { letter: "P.01", title: isEn ? "FOUNDATIONS & SCALE" : "CIMIENTOS Y ESCALA", desc: isEn ? "We do not only intervene in consolidated companies with outdated ecosystems. We accompany new businesses to be born from day zero with an unbreakable digital infrastructure." : "No solo intervenimos empresas consolidadas con ecosistemas desfasados. Acompañamos a nuevos negocios a nacer desde el día cero con una infraestructura digital inquebrantable.", icon: <svg aria-hidden="true" className="w-8 h-8 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M3 17l6-6 4 4 8-8M21 7v6M21 7h-6" /></svg> },
   ...
   <div className="absolute inset-0 z-0 bg-gradient-to-b from-transparent via-black/40 to-black"></div>
            ...
            <ScrollReveal delay={0.15}>
              <div className="font-space-mono text-[#F5B700] text-[10px] md:text-xs tracking-[0.2em] mb-6 uppercase">
                {isEn ? "[MODULE 01] // STATUS: ACTIVE // TECHNICAL REPORT: DEVELOPMENT" : "[MÓDULO 01] // ESTADO: ACTIVO // REPORTE TÉCNICO: DESARROLLO"}
              </div>
            </ScrollReveal>
 ...
 <h1 className="hero-stroke font-clash font-bold uppercase mb-8 md:mb-12 text-[10vw] sm:text-[8vw] md:text-6xl lg:text-[5rem] xl:text-[6.5rem] break-words leading-[1.1] md:leading-[1.1] flex flex-col w-full max-w-5xl border-l-0 md:border-l-[6px] border-[#F5B700] pl-0 md:pl-10"> 
                <div className="overflow-hidden">
                    <ScrollReveal variant="textReveal" delay={0.1} className="">
                        {isEn ? "HIGH-PERFORMANCE" : "DESARROLLO WEB"}
                      </ScrollReveal>
                  </div>
                  <div className="overflow-hidden pb-4">
                    <ScrollReveal variant="textReveal" delay={0.2} className="">
                      {isEn ? "WEB DEVELOPMENT" : "DE ALTO RENDIMIENTO"}<span className="">.</span>
                    </ScrollReveal>
                  </div>
              </h1>
              ...
              <div className="overflow-hidden mb-12 md:mb-20 pt-4">
                <ScrollReveal delay={0.3}>
                  ...
                  <p className="font-inter font-normal text-zinc-400 text-base md:text-xl lg:text-2xl max-w-4xl leading-relaxed md:leading-tight tracking-tight">
                    {isEn ? <>The digital <span className="text-[#F5B700]">engine</span> of your company. Zero errors, maximum <span className="text-[#F5B700]">security</span> and a performance designed exclusively to shield your <span className="text-[#F5B700]">sales</span>.</> : <>El <span className="text-[#F5B700]">motor</span> digital de tu empresa. Cero errores, máxima <span className="text-[#F5B700]">seguridad</span> y un rendimiento diseñado exclusivamente para blindar tus <span className="text-[#F5B700]">ventas</span>.</>}
                  </p>
                </ScrollReveal>
              </div>

              <ScrollReveal delay={0.8}>
                <ContactTrigger servicio="web" className="inline-block cursor-pointer">
                  <button type="button" className="rings-btn">
                    <i></i><i></i><i></i>
                    <span>{isEn ? "Request Web Audit" : "Solicitar Auditoría Web"}</span>
            ...
            <div className="absolute bottom-8 right-6 md:bottom-12 md:right-12 z-20">
              <ScrollReveal delay={1.0}>
                <Link href={isEn ? "/en/manifesto" : "/es/manifiesto"} className="relative inline-block text-[#888888] font-clash font-semibold text-[10px] md:text-xs pb-1 group transition-colors hover:text-white whitespace-nowrap">
 ...
 <h2 className="font-clash font-light text-[4vw] sm:text-lg md:text-2xl lg:text-3xl text-[#F5B700] uppercase whitespace-nowrap"> 
              ...
              <div className="leading-[1.1] md:leading-[1.1] flex flex-col">
                {[
                  { id: "01", title: isEn ? "Forensic Audit" : "Auditoría Forense", desc: isEn ? "Exact radiograph of your operational bottlenecks. We detect where your current system fails and where your company's capital is leaking." : "Radiografía exacta de tus cuellos de botella operativos. Detectamos dónde falla tu sistema actual y por dónde se está fugando el capital de tu empresa." },
                  { id: "02", title: isEn ? "Demolition & Architecture" : "Demolición y Arquitectura", desc: isEn ? "We eliminate the unstable systems holding your business back. We deploy custom, fast, and secure technology designed so you operate automatically and without server crashes." : "Eliminamos los sistemas inestables que frenan tu negocio. Desplegamos tecnología a medida, rápida y segura, diseñada para que operes en automático y sin caídas de servidor." },
                                    { id: "03", title: isEn ? "Digital Governance" : "Gobernanza Digital", desc: isEn ? "We do not deliver orphan software. We apply a 'Comprehensive Tech Insurance': monthly maintenance, protection, and continuous evolution." : "No entregamos software huérfano. Aplicamos un 'Seguro a Todo Riesgo Tecnológico': mantenimiento, protección y evolución continua mensual." },
                ].map((phase, i) => (
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "FORENSIC AUDIT" : "AUDITORÍA FORENSE"}
                </span>
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "DEMOLITION & ARCHITECTURE" : "DEMOLICIÓN Y ARQUITECTURA"}
                </span>
                <svg aria-hidden="true" viewBox="0 0 24 24" className="text-[#F5B700] w-8 h-8 md:w-12 md:h-12 shrink-0 pb-1" style={{ stroke: 'currentColor', strokeWidth: 1.5, fill: 'none' }}>
                  <line x1="12" y1="2" x2="12" y2="22" />
                  <line x1="2" y1="12" x2="22" y2="12" />
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "INTELLIGENT AUTOMATION" : "AUTOMATIZACIÓN INTELIGENTE"}
                </span>
                <svg aria-hidden="true" viewBox="0 0 24 24" className="text-[#F5B700] w-8 h-8 md:w-12 md:h-12 shrink-0 pb-1" style={{ stroke: 'currentColor', strokeWidth: 1.5, fill: 'none' }}>
                  <line x1="12" y1="2" x2="12" y2="22" />
                  <line x1="2" y1="12" x2="22" y2="12" />
 ...
 <span className="font-clash font-semibold text-transparent text-3xl md:text-5xl lg:text-[4.5rem] uppercase mx-4 md:mx-8" style={{ WebkitTextStroke: '1px rgba(255,255,255,0.4)', color: 'transparent' }}> 
                  {isEn ? "DIGITAL GOVERNANCE" : "GOBERNANZA DIGITAL"}
                </span>
